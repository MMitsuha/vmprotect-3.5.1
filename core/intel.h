...
static const char *intel_command_name[] = {"db ??",
                                           "push",
                                           "pop",
                                           "mov",
                                           "add",
...
struct IntelOperand {
  ...
  IntelSegment effective_base_segment(IntelSegment base_segment) const {
    if (base_segment == segDefault &&
        (((type & otBaseRegistr) &&
          (base_registr == regESP || base_registr == regEBP)) ||
  ...
  IntelOperand(uint32_t type_, OperandSize size_, uint8_t registr_ = 0,
               uint64_t value_ = 0, IFixup *fixup_ = NULL);

  uint64_t encode() const {
    uint64_t res = static_cast<uint64_t>(type) << 48;
    if (type &
        (otRegistr | otSegmentRegistr | otControlRegistr | otDebugRegistr |
         otFPURegistr | otHiPartRegistr | otMMXRegistr | otXMMRegistr))
...
public:
  explicit IntelVMCommand(IntelCommand *owner, IntelCommandType command_type,
                          OperandType operand_type, OperandSize size,
                          uint64_t value, uint32_t options);
  ...
  void set_crypt_command(IntelCommandType crypt_command, OperandSize crypt_size,
                         uint64_t crypt_key) {
    crypt_command_ = crypt_command;
    crypt_size_ = crypt_size;
  ...
  bool is_data() const {
    return (command_type_ == cmDD || command_type_ == cmDQ);
  }
...
public:
  ...
  virtual void Add(AccessType access_type, uint8_t value,
                   OperandType operand_type, OperandSize size);
  void AddOperand(const IntelOperand &operand, AccessType access_type);
  void set_base_segment(IntelSegment base_segment) {
...
public:
  explicit IntelCommand(IFunction *owner, OperandSize size,
                        uint64_t address = 0);
  explicit IntelCommand(IFunction *owner, OperandSize size,
                        IntelCommandType type,
                        IntelOperand operand1 = IntelOperand(),
                        IntelOperand operand2 = IntelOperand(),
                        IntelOperand operand3 = IntelOperand());
  explicit IntelCommand(IFunction *owner, OperandSize size,
                        const std::string &value);
  explicit IntelCommand(IFunction *owner, OperandSize size,
                        const os::unicode_string &value);
  ...
  virtual size_t original_dump_size() const {
    return (original_dump_size_) ? original_dump_size_ : dump_size();
  }
  IntelSegment base_segment() const { return base_segment_; }
  ...
  void set_preffix_command(IntelCommandType preffix_command) {
    preffix_command_ = preffix_command;
  }
  void Init(IntelCommandType type, IntelOperand operand1 = IntelOperand(),
            IntelOperand operand2 = IntelOperand(),
            IntelOperand operand3 = IntelOperand());
  ...
  virtual void include_section_option(SectionOption option) {
    section_options_ |= option;
  }
  virtual void exclude_section_option(SectionOption option) {
    section_options_ &= ~option;
  }
  IntelVMCommand *AddVMCommand(const CompileContext &ctx,
                               IntelCommandType command_type,
                               OperandType operand_type, OperandSize size,
                               uint64_t value, uint32_t options = 0,
                               IFixup *fixup = NULL);
  void AddBeginSection(const CompileContext &ctx, uint32_t options = 0);
  void AddEndSection(const CompileContext &ctx, IntelCommandType end_command,
                     uint8_t end_value = 0, uint32_t options = 0);
  ...
  void AddCryptorSection(const CompileContext &ctx, ValueCryptor *cryptor,
                         bool is_decrypt);
...
private:
  ...
  // disasm methods
  void ReadCommand(IntelCommandType type, uint32_t of_1, uint32_t of_2,
                   uint32_t of_3, DisasmContext &ctx);
  void ReadRegFromRM(uint8_t code, OperandSize operand_size,
                     OperandType operand_type, const DisasmContext &ctx);
  void ReadRM(uint8_t code, OperandSize operand_size, OperandType operand_type,
              bool show_size, const DisasmContext &ctx);
  void ReadReg(uint8_t code, OperandSize operand_size, OperandType operand_type,
               const DisasmContext &ctx);
  IntelOperand *ReadValue(OperandSize operand_size, OperandSize value_size,
                          const DisasmContext &ctx);
  void ReadValueAddAddress(OperandSize operand_size, OperandSize value_size,
                           const DisasmContext &ctx);
  ...
  void PushRegAndRM(size_t reg_operand_index, size_t rm_operand_index,
                    AsmContext &ctx);
  ...
  void CompileOperand(const CompileContext &ctx, size_t operand_index,
                      uint32_t options = 0);
  void AddCorrectOperandSizeSection(const CompileContext &ctx, OperandSize src,
                                    OperandSize dst);
  void AddCombineFlagsSection(const CompileContext &ctx, uint16_t mask);
  void AddRegistrAndValueSection(const CompileContext &ctx, uint8_t registr,
                                 OperandSize registr_size, uint64_t value,
                                 bool need_pushf = false);
  void AddRegistrOrValueSection(const CompileContext &ctx, uint8_t registr,
                                OperandSize registr_size, uint64_t value,
                                bool need_pushf = false);
  void AddCorrectFlagSection(const CompileContext &ctx, uint16_t flags);
  void AddExtractFlagSection(const CompileContext &ctx, uint16_t flags,
                             bool is_inverse, uint8_t extract_to);
  void AddJmpWithFlagSection(const CompileContext &ctx,
                             IntelCommandType command_type);
  void AddCorrectESPSection(const CompileContext &ctx, OperandSize operand_size,
                            size_t value);
  void AddCheckBreakpointSection(const CompileContext &ctx,
                                 OperandSize address_size);
...
public:
  explicit SectionCryptor(SectionCryptorList *owner,
...
public:
  explicit IntelFunction(IFunctionList *owner, const std::string &name,
                         CompilationType compilation_type,
                         uint32_t compilation_options, bool need_compile,
                         Folder *folder);
  explicit IntelFunction(IFunctionList *owner = NULL);
  explicit IntelFunction(IFunctionList *owner, OperandSize cpu_address_size,
  ...
  IntelCommand *item(size_t index) const {
    return reinterpret_cast<IntelCommand *>(IFunction::item(index));
  }
  IntelCommand *ReadValidCommand(IArchitecture &file, uint64_t address);
  void ReadMarkerCommands(IArchitecture &file, MarkerCommandList &command_list,
                          uint64_t address, uint32_t options);
  ...
  IntelCommand *AddCommand(IntelCommandType type,
                           IntelOperand operand1 = IntelOperand(),
                           IntelOperand operand2 = IntelOperand(),
                           IntelOperand operand3 = IntelOperand());
  ...
  bool ParseCompressedCxxSEH(IArchitecture &file, uint64_t address,
                             uint64_t begin);
  bool ParseScopeSEH(IArchitecture &file, uint64_t address,
                     uint32_t table_count);
  virtual IntelCommand *ParseCommand(IArchitecture &file, uint64_t address,
                                     bool dump_mode = false);
  uint64_t ParseParam(IArchitecture &file, size_t index,
                      uint64_t &param_reference);

protected:
  virtual IntelCommand *CreateCommand();
  virtual IntelCommand *ParseString(IArchitecture &file, uint64_t address,
                                    size_t len);
  ...
  virtual IntelFunction *CreateFunction(IFunction *parent = NULL) {
    return new IntelFunction(NULL, cpu_address_size(), parent);
  }
...
private:
  ...
  bool ParseSwitch(IArchitecture &file, uint64_t address,
                   OperandSize value_size, uint64_t add_value,
                   IntelCommand *parent_command, size_t mode,
  ...
  bool ParseBCBSEH(IArchitecture &file, uint64_t address, uint64_t next_address,
                   uint8_t version);
  bool ParseDelphiSEH(IArchitecture &file, uint64_t address);
  CompilerFunction *ParseCompilerFunction(IArchitecture &file,
                                          uint64_t address);
...
enum ValueType { vtNone = 0, vtValue = 1, vtRegistr = 2, vtReturnAddress = 4 };
...
public:
  ...
  void Calc(IntelCommandType command_type, uint16_t command_flags,
            bool inverse_flags, OperandSize size, uint64_t op2,
            IntelFlagsValue *flags);
...
public:
  ...
  void Calc(IntelCommandType command_type, OperandSize size, uint64_t op1,
            uint64_t op2, uint64_t result);
...
public:
  ...
  void Compile(
      IntelFunction *func,
      size_t
          index /*, size_t end_index = -1, bool for_virtualization = false*/);
...
private:
  void AddMarker(IArchitecture &file, uint64_t address, uint64_t name_reference,
                 uint64_t name_address, ObjectType type, uint8_t tag,
                 bool is_unicode);
  void AddString(IArchitecture &file, uint64_t address, uint64_t reference,
                 bool is_unicode);
  void AddEndMarker(IArchitecture &file, uint64_t address,
                    uint64_t next_address, ObjectType type);
...
public:
  ...
  size_t table_size() const {
    return (count() - 2) * OperandSizeToValue(osDWord);
  }
...
public:
  explicit IntelRuntimeCRCTable(IFunctionList *owner,
                                OperandSize cpu_address_size);
  virtual void clear();
  virtual bool Compile(const CompileContext &ctx);
  virtual size_t WriteToFile(IArchitecture &file);
...
public:
  ...
  explicit IntelFunctionList(IArchitecture *owner,
                             const IntelFunctionList &src);
  ~IntelFunctionList();
  virtual IntelFunctionList *Clone(IArchitecture *owner) const;
  virtual IntelFunction *Add(const std::string &name,
                             CompilationType compilation_type,
                             uint32_t compilation_options, bool need_compile,
                             Folder *folder);
  ...
  virtual IntelFunction *
  CreateFunction(OperandSize cpu_address_size = osDefault);
...
private:
  ...
  IntelFunction *AddWatermark(OperandSize cpu_address_size,
                              Watermark *watermark, int copy_count);
...
public:
  ...
  virtual_machine(IVirtualMachineList *virtual_machine_list,
...
protected:
  struct LoaderInfo {
    IntelCommand *data;
    size_t size;
    LoaderInfo(IntelCommand *data_, size_t size_) : data(data_), size(size_) {}
  };
...
public:
  ...
  IntelCommand *cfg_check_function_entry() const {
    return cfg_check_function_entry_;
  }
...
private:
  ...
  struct ImportFunctionInfo {
    ...
    ImportFunctionInfo(PEImportFunction *import_function_)
        : import_function(import_function_), name(NULL), thunk(NULL),
          loader_name(NULL) {}
  ...
  struct PackerInfo {
    ...
    size_t size;
    IntelCommand *data;
    bool operator==(PESegment *section_) const { return (section == section_); }
  };
};
...
public:
  ...
  explicit MacIntelFunctionList(IArchitecture *owner,
                                const MacIntelFunctionList &src);
...
public:
  ...
  IntelCommand *thread_variables_entry() const {
    return thread_variables_entry_;
  }
  ...
  std::vector<MacSegment *> packed_segment_list() const {
    return packed_segment_list_;
  }
...
private:
  ...
  struct PackerInfo {
    ...
    MacSegment *segment;
    IntelCommand *data;
    PackerInfo() : address(0), size(0), segment(NULL), data(NULL) {}
...
public:
  ...
  explicit ELFIntelFunctionList(IArchitecture *owner,
                                const ELFIntelFunctionList &src);
...
public:
  ...
  IntelCommand *term_entry() const {
    return term_entry_ ? reinterpret_cast<IntelCommand *>(
                             term_entry_->link()->to_command())
                       : NULL;
  }
...
public:
  IntelOpcodeInfo(IntelOpcodeList *owner, IntelCommandType command_type,
                  OperandType operand_type, OperandSize size, uint8_t value,
                  IntelCommand *entry, OpcodeCryptor *value_cryptor = NULL,
                  OpcodeCryptor *end_cryptor = NULL);
  ~IntelOpcodeInfo();
  IntelCommandType command_type() const { return command_type_; }
  OperandType operand_type() const { return operand_type_; }
  OperandSize size() const { return size_; }
  ...
  uint64_t Key();
  static uint64_t Key(IntelCommandType command_type, OperandType operand_type,
                      OperandSize size, uint8_t value);
...
public:
  IntelOpcodeList();
  IntelOpcodeInfo *Add(IntelCommandType command_type, OperandType operand_type,
                       OperandSize size, uint8_t value,
                       IntelCommand *entry = NULL,
                       OpcodeCryptor *value_cryptor = NULL,
                       OpcodeCryptor *end_cryptor = NULL);
  IntelOpcodeInfo *GetOpcodeInfo(IntelCommandType command_type,
                                 OperandType operand_type, OperandSize size,
                                 uint8_t value) const;
};
...
enum VirtualMachineType { vtClassic, vtAdvanced };

struct IntelVirtualMachineObfuscation {
  IntelCommand *begin_;
  IntelCommand *end_;

  IntelVirtualMachineObfuscation(IntelCommand *begin, IntelCommand *end)
      : begin_(begin), end_(end) {}
};

class IntelVirtualMachineProcessor : public IntelFunction {
public:
  IntelVirtualMachineProcessor(IntelFunctionList *parent,
                               OperandSize cpu_address_size);
  virtual bool Prepare(const CompileContext &ctx);
  ...
  std::vector<std::unique_ptr<IntelVirtualMachineObfuscation>>
      obfuscation_list_;
};
...
public:
  IntelVirtualMachine(IntelVirtualMachineList *owner, VirtualMachineType type,
                      uint8_t id, IntelVirtualMachineProcessor *processor);
  ~IntelVirtualMachine();
  void Init(const CompileContext &ctx,
            const IntelOpcodeList &visible_opcode_list);
  ...
  ValueCryptor *entry_cryptor() const {
    return &const_cast<ValueCryptor &>(entry_cryptor_);
  }
...
private:
  IntelCommand *AddReadCommand(OperandSize size, OpcodeCryptor *command_cryptor,
                               uint8_t registr);
  void AddValueCommand(ValueCommand &value_command, bool is_decrypt,
                       uint8_t registr);
  void AddEndHandlerCommands(IntelCommand *to_command,
                             OpcodeCryptor *command_cryptor);
  IntelCommand *CloneHandler(IntelCommand *handler);
  void InitCommands(const CompileContext &ctx,
                    const IntelOpcodeList &visible_opcode_list);
  void AddCallCommands(CallingConvention calling_convention,
                       IntelCommand *call_entry, uint8_t registr);
  IntelOpcodeInfo *GetOpcode(IntelCommandType command_type,
                             OperandType operand_type, OperandSize size,
                             uint8_t value);
...
public:
  ...
  IntelVirtualMachine *item(size_t index) const {
    return reinterpret_cast<IntelVirtualMachine *>(
        IVirtualMachineList::item(index));
  }
